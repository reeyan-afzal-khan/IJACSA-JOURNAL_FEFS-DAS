...
report_dir = C.results_dir("shared", "tables")
fig_dir = C.results_dir("shared", "figures")
...
stats = {}
for ds in DATASETS:
    path = C.results_dir(ds, "06_benchmark" + suffix) / "benchmark_full.csv"
    if not path.exists():
...
"""),
    ("code", """
RUN_CNN = False     # set True once the full extraction has been run
EPOCHS, BATCH = 20, 256

if not RUN_CNN:
    print("RUN_CNN = False. The CNN baseline re-reads raw windows and is only")
    print("meaningful on the full (non-smoke) extraction. Set RUN_CNN = True to run it.")
else:
    ...
    for ds in DATASETS:
        ...
        mani = mani.groupby(["label", "split"], group_keys=False).apply(
            lambda g: g.sample(min(len(g), 4000), random_state=C.SEED))

        # Load raw windows (decimated to the CNN's 2048-sample input).
        Xs, ys, ss = [], [], []
        for _, r in mani.iterrows():
            if ds == "cao":
                raw = pp.load_cao_window(r.path, spec.win_len)
                x = pp.preprocess_patch(raw, spec.fs)[pp.cao_reference_channel(raw)]
            else:
                with pp.TomasovReader(r.h5_path) as rd:
                    raw = rd.read_patch(int(r.t0), int(r.locus), spec.win_len, spec.patch_channels)
                x = pp.preprocess_patch(raw, spec.fs)[pp.center_channel_index(spec.patch_channels)]
            Xs.append(x[::4][:2048]); ys.append(r.label); ss.append(r.split)
        Xs = np.asarray(Xs, dtype=np.float32)[:, None, :]
        classes = sorted(set(ys))
        yi = np.array([classes.index(v) for v in ys])
        ss = np.array(ss)

        net = MD.build_cnn_baseline(len(classes), 2048).to(dev)
        w = torch.tensor(1.0/np.bincount(yi[ss=="train"], minlength=len(classes)),
        ...
        dl = DataLoader(TensorDataset(torch.tensor(Xs[ss=="train"]),
                                      torch.tensor(yi[ss=="train"])),
        ...
        for ep in range(EPOCHS):
            ...
            for xb, yb in dl:
                ...
                tot += float(l) * len(xb)
            sched.step(tot)
            if (ep+1) % 5 == 0: print(f"  {ds} epoch {ep+1}/{EPOCHS} loss {tot/(ss=='train').sum():.4f}")
        train_s = time.perf_counter() - t0

        net.eval()
        with torch.no_grad():
            pred = net(torch.tensor(Xs[ss=="test"]).to(dev)).argmax(1).cpu().numpy()
        met = EV.score_all(yi[ss=="test"], pred)
        cnn_rows.append({"dataset": ds, "model": "1D-CNN (DAS_CNN)",
                         "n_train": int((ss=="train").sum()), "n_test": int((ss=="test").sum()),
                         "accuracy": 100*met["accuracy"], "f1_macro": 100*met["f1_macro"],
                         "train_seconds": round(train_s, 1),
                         "n_params": sum(p.numel() for p in net.parameters())})
        print(f"{ds}: CNN accuracy {100*met['accuracy']:.2f}%  macro-F1 {100*met['f1_macro']:.2f}%")
...
for ds in DATASETS:
    ...
    cpath = C.results_dir(ds, "07_final" + suffix) / "model_card.json"
    if not tpath.exists():
        continue
...
for ds in DATASETS:
    cpath = C.results_dir(ds, "07_final" + suffix) / "model_card.json"
    bpath = C.results_dir(ds, "06_benchmark" + suffix) / "best_config.json"
    if not cpath.exists():
        continue
    card = json.loads(cpath.read_text())
